...
class AsyncExecutor:
    ...
    async def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            # Log when client stops execution
            total_duration = time.perf_counter() - total_start
            console.println(f" Client id {self.client_id} completed task [{self.task.name}] after {total_duration:.1f}s")
            self.logger.info("Client [%s] completed task [%s] after %.2f seconds.", self.client_id, self.task.name, total_duration)
    ...
    if requires_time_period_schedule(task, runner_for_op, params_for_op):
        ...
        if client_index == 0:
            logger.info("Creating time-period based schedule with [%s] distribution for [%s] with a warmup period of [%s] "
                        "seconds, a time period of [%s] seconds, and a ramp-down period of [%s] seconds.",
                        task.schedule, task.name, str(warmup_time_period), str(task.time_period), str(ramp_down_time_period))
